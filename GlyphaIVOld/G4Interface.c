...
static void DoGameSettings (void);
...
void MenusReflectMode (void)
{
	...
	{
		DisableMenuItem(gameMenu, iNewGame);		// Cannot begin another New Game.
		EnableMenuItem(gameMenu, iPauseGame);		// Can Pause Game.
		if (pausing) {							// If we are paused…
			SetMenuItemText(gameMenu, iPauseGame, "\pResume Game");			// Rename item "Resume Game".
		...
		EnableMenuItem(gameMenu, iEndGame);			// Can End Game.
		DisableMenuItem(optionsMenu, 0);			// Cannot change game settings.
	}
	else										// Else, if Glypha is idle…
	{
		EnableMenuItem(gameMenu, iNewGame);			// Can begin a New Game.
		DisableMenuItem(gameMenu, iPauseGame);		// Cannot Pause Game.
		SetMenuItemText(gameMenu, iPauseGame, 
				"\pPause Game");				// Rename item "Pause Game".
		DisableMenuItem(gameMenu, iEndGame);		// Cannot End Game.
		EnableMenuItem(optionsMenu, 0);				// Can change game settings.
	}
}
...
void HandleKeyEvent (EventRecord *theEvent)
{
	...
	if ((commandDown) && (theEvent->what == keyDown))								// If command key down, call menu…
	{												// handling routine.
		if (canPlay)
...
void DoAbout (void)
{
	...
	aboutWindow = GetNewCWindow(129, NULL, kPutInFront);
